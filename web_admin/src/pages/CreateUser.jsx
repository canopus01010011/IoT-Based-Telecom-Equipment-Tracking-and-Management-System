    ...
    <PageLayout title={t.createUserTitle} maxWidth="max-w-2xl">
      ...
      <form onSubmit={handleSubmit}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
            {ROLES.map(r => (
              <div key={r.id} onClick={() => setRole(r.id)}
                style={{ border: role === r.id ? `1.5px solid ${r.color}` : '0.5px solid rgba(59,130,246,.15)', background: role === r.id ? r.bg : 'transparent', borderRadius:10, padding:16, cursor:'pointer', transition:'all .15s' }}>
        ...
        {role === 'technician' && (
          <FormCard title={t.technicianInfo}>
            ...
            <select name="specialite" value={form.specialite} onChange={set} style={{ ...inp, cursor:'pointer' }}>
              <option value=""                      style={{ background:'#0d1426' }}>{t.chooseSpecialty}</option>
              <option value="Fiber Optics"          style={{ background:'#0d1426' }}>{t.fiberOptics}</option>
              <option value="Antenna / Tower"       style={{ background:'#0d1426' }}>{t.antennasTower}</option>
              <option value="Network Cabling"       style={{ background:'#0d1426' }}>{t.networkCabling}</option>
              <option value="Generator"             style={{ background:'#0d1426' }}>{t.generator}</option>
              <option value="Site Air Conditioning" style={{ background:'#0d1426' }}>{t.siteAC}</option>
              <option value="Multi-skilled"         style={{ background:'#0d1426' }}>{t.multiskilled}</option>
            </select>
          </FormCard>
        )}
        ...
        <FormCard title={t.passwordSection}>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14 }}>
            <div><label style={lbl}>{t.password}</label><input type="password" name="password" value={form.password} onChange={set} placeholder="••••••••" required style={inp} /></div>
            <div><label style={lbl}>{t.confirm}</label><input type="password" name="confirm" value={form.confirm} onChange={set} placeholder="••••••••" required style={inp} /></div>
          </div>
